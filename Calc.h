...
inline string diff_calc_fu(string str,string var_name)
{
	...
	while (!q.empty())
	{
		...
		else if (isdigit(token[0])|| (token[0] == '-' && isdigit(token[1])) || token=="i")
		{
			...
			{
				if (static_cast<ll>(n.imag() != n.imag()))stk.push({to_string(n.imag())+"i","0"});
				else stk.push({to_string(ll(n.imag()))+"i","0"});
			}
			else stk.push({"("+to_string(n.real())+(n.imag()<0?"-":"+")+to_string(abs(n.imag()))+"i)","0"});
		}
		else if (prio(token)<4)
		{
			if (stk.size() < 2) {
			...
			else if (token == "^")
			{
				...
				for (auto v:l.first)if (!isdigit(v)&&v!='.'&&v!='i'&&v!='+'&&v!='('&&v!=')'&&v!='*')
				{
					if (l.first.size()!=1)first="("+l.first+")";
					else first=l.first;
				}
				if (first.empty())first=l.first;
				for (auto v:r.first)if (!isdigit(v)&&v!='.'&&v!='i'&&v!='+'&&v!='('&&v!=')'&&v!='*')
				{
					if (r.first.size()!=1)first+="^("+r.first+")";
					else first+="^"+r.first;
					f2=1;
				}
				if (!f2)first+="^"+r.first;
				...
				// cout << type1 << " " << type2 << "\n";
				if (!type1&&!type2)second="0";
				else if (type1&&!type2) // x^a r.first是常数
				{
					cld cc = complex_calc(r.first);
					if (l.first.size()!=1)
					{
						if (cc.imag()==0)second = r.first+"*("+l.first+")";
						else second = "("+to_string(cc.real())+"+"+to_string(cc.imag())+"i)*("+l.first+")";
					}
					else
					{
						if (cc.imag()==0)second = r.first+l.first;
					...
					if (cc!=cld(2))
					{
						if (cc.imag()==0) second += "^"+to_string(cc.real()-1); // 实数
						else second += "^("+to_string(cc.real()-1)+"+"+to_string(cc.imag())+"i)";
					}
				}
				else if (!type1&&type2) // a^x, l.first^r.first l.first^r.first*ln(l.first)*r.second
				{
					cld cc = complex_calc(l.first);
					ld real = cc.real(), imag = cc.imag();
					if (imag==0)
					{
						if (ll(real)==real)second = to_string(ll(real))+"^";
						else second = to_string(cc.real())+"^";
						if (r.first.size()==1)second+=r.first;
						else second+="("+r.first+")";
						if (ll(real)!=real)second+="*ln("+to_string(cc.real())+")";
						else second += "*ln("+to_string((ll)cc.real())+")";
						if (r.second!="1")second+="("+r.second+")";
					}else
					{
						second = "("+to_string(cc.real())+"+"+to_string(cc.imag())+"i)^";
						if (r.first.size()==1)second+=r.first;
						else second+="("+r.first+")";
						second+="*ln("+to_string(cc.real())+"+"+to_string(cc.imag())+"i)";
						if (r.second!="1")second+="("+r.second+")";
					}
					if (cc==cld(1)||cc==cld(0))second="0";
				}
				else // f(x)^g(x)
				{
					if (l.first.size()==1)second+=l.first+"^";
					else second+="("+l.first+")^";
					if (r.first.size()==1)second+=r.first+"*(";
					else second+="("+r.first+")*(";
					if (l.first!="1"){
						if (r.second.find_first_not_of("0123456789") != string::npos)second+="("+r.second+")";
						else if (r.second!="1")second+=r.second;
						if (r.second!="0")second+="ln("+l.first+")+";
					}
					if (r.first==l.first)second+="1";
					else
					{
						if (r.first.size()!=1)second+="("+r.first+")";
						else second+=r.first;
						if (l.second.size()!=1)second+="("+l.second+")";
						else if (l.second!="1")second+="*"+l.second;
						second+="/";
						if (l.first.size()!=1)second+="("+l.first+")";
						else second+=l.first;
					}
